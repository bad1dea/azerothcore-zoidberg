...
### 4. Warrior offensive-ability cast (spell 78) rejected -- cause not yet confirmed
...
live Mottled Boar via `.autonomousplayer castspell`, both early and
mid-combat, while genuinely engaged (real melee damage was landing from
auto-attack). Rejected both times (`accepted=false`, target HP
unchanged). **Root cause not confirmed** -- three real possibilities, not
ruled out: insufficient rage (a real cost for this ability in real WoW,
generated by combat, and the casts were tried early when rage would
still be low), a wrong spell-ID assumption, or the ability being a real
"queued for next melee swing" mechanic that may not work correctly
through `Combat::RequestCastSpell`'s current `Unit::CastSpell(...,
triggered=false)` call (which has only ever been tested before against a
genuinely non-castable spell, never a real successful non-instant
combat-ability cast). Not chased further this session -- see
`HANDOFF.md` NEXT TASK for concrete next diagnostic steps (expose the
