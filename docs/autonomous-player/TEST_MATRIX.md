...
| `KillNearest` engagement confirmation fixed (`GetVictim()==target`, not `IsInCombat()`) — real bug found via external review | Fixed by reasoning + code review; happy path re-verified correctly live (real target, correct guid, real melee range). Dedicated attempts to force the exact negative case (unrelated attacker present) via `multipull`+`guidestartcombat` overlap could not reliably reproduce sustained overlap — test creatures die faster than console-command latency | Fixed, happy path re-verified; **the specific scenario the fix targets not yet proven by direct live observation** — see `KNOWN_FAILURES.md` #5 |
...
| Dense camps/caves, ranged pulls, pets, full bags, broader guide validation, more race/class combos, class controllers (research doc step 3), EncounterModel actually influencing behavior, bounded failure states for quest/MoveTo steps | Not yet attempted | Deferred, `HANDOFF.md` `NEXT TASK` |
