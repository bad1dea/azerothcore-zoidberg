...
| `KillNearest` against a target found near/past the 100-yard search radius edge | Live on zoidberg, 3 separate attempts (original code, arrival-gate fix, MoveChase fix): bot got permanently stuck each time, different stall position each time, no target findable within 100 yards afterward | **Known failure, unresolved** — see `KNOWN_FAILURES.md` #3, `HANDOFF.md` NEXT TASK |
| `AcceptQuest`/`TurnInQuest` guide steps: interaction-range gating, already-active-quest fast path | Live: `AcceptQuest` step correctly advanced instantly when the target quest was already active/incomplete in the bot's log (`GetQuestStatus != NONE`) | Verified (this specific behavior only — see below for the full chain) |
| Full automatic quest loop (AcceptQuest→KillNearest→TurnInQuest, one guide, zero manual steps) | Attempted live 3 times via `guidestartquest`; blocked by the open `KillNearest` bug above each time — full clean completion **not yet observed** | Blocked, not yet verified |
