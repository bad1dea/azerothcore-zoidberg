...
void VisibleNotifier::SendToSelf()
{
    ...
    VisibleWorldObjectsSet* visibleWorldObjects = i_player.GetObjectVisibilityContainer().GetVisibleWorldObjectsSet();
    for (VisibleWorldObjectsSet::iterator itr = visibleWorldObjects->begin(); itr != visibleWorldObjects->end();)
    {
        // The container stores GUIDs, not raw pointers. Resolve each through the live
        // object registry: a freed/removed object resolves to nullptr (or is no longer
        // in world), so we drop the stale entry instead of dereferencing freed memory.
        WorldObject* obj = ObjectAccessor::GetWorldObject(i_player, *itr);
